...
def generate_motion_samples(
    ...
    radius_meshgrid = torch.stack([
        torch.stack(torch.meshgrid(2 * [torch.linspace(-r_i, r_i, samples)], indexing='ij'), dim=-1)
        for r_i in r
    ], dim=0).to(q_samples.device) # N x step x step x 2
...
class DragGAN:
    ...
    def norm2pixel(self, p: torch.Tensor):
        scale_factor = self._input_size / 2.
    ...
    def get_w_latent_from_seed(self, seed: int, truncation_psi: int = 0.85) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            with warnings.catch_warnings(record=True) as warning:
                warnings.simplefilter("ignore")
                w_latent_orig = self._G.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=None).detach().clone()
    ...
    def _compute_motion_step(
        ...
        for step_idx in range(steps):
            ...
            Fq = torch.nn.functional.grid_sample(F_exp.float(), qr1_samples.float(), mode='bilinear', align_corners=False).detach()

            # F(q + d)
            # Use grid_sample to extract the features for q + d, where q € neighbours(p, r1)
            Fqd = torch.nn.functional.grid_sample(F_exp.float(), qr1d_samples.float(), mode='bilinear', align_corners=False)
    ...
    def _compute_point_tracking(
        ...
        f_i = torch.nn.functional.grid_sample(F0_exp.float(), p_init_samples.float(), mode='bilinear', align_corners=False)
    ...
    def init(
        ...
        if mask_in_pixels is not None:
            ...
            M = generate_motion_masks(mask_in_pixels, output_size=self._features_extractor_size, dims=self._features_extractor_dims).to(self._device)
